        ...
        form = FORM(H2('One. Last. Step.'),
                    ...
                    DIV(H4('Card details'),
                        DIV(LABEL('Card Number', _for='card_num'),
                            INPUT(_name='card_num', _type='number', _placeholder='XXXXXXXXXXXX',
                    ...
                    DIV(H4('Billing Address'),
                        ...
                        DIV(LABEL('Post Code', _for='bill_postcode'),
                            INPUT(_name='bill_postcode',
                                  ...
                                  _value=session.signup['address.postcode'], _placeholder='XXXX XXX')),
                        _class='right_div'),
                    DIV(_class='clear'),
...
def view():
    ...
    form = FORM(H3('Login info'),
                DIV(DIV(LABEL('Username', _for='username'),
                    INPUT(_name='username',
                          requires=IS_NOT_IN_DB(db, 'users.username', allowed_override=db_profile.username,
                                                error_message='Sorry, that username already exists.'),
                          _value=db_profile.username)),
                DIV(LABEL('Password', _for='password'),
                    INPUT(_name='password', _type='password',
                          requires=IS_NOT_EMPTY(error_message='Please set yourself a non-empty password.'),
                          _value=db_profile.pwd)),
                DIV(LABEL('Confirm Password', _for='confirm_pwd'),
                    INPUT(_name='confirm_pwd', _type='password',
                          requires=IS_EQUAL_TO(request.vars.password, error_message='Set passwords must match.'),
                          _value=db_profile.pwd)),
                H3('Personal Info'),
                DIV(LABEL('First Name', _for='firstname'),
                    INPUT(_name='firstname', requires=IS_NOT_EMPTY(error_message='Please give your first name.'),
                          _value=db_profile.firstname)),
                DIV(LABEL('Last Name', _for='lastname'),
                    INPUT(_name='lastname', requires=IS_NOT_EMPTY(error_message='Please give your last name.'),
                ...
                DIV(H4('Address'),
                    ...
                    DIV(LABEL('Road name', _for='street'),
                        INPUT(_name='street',
                              requires=IS_NOT_EMPTY(error_message='Please enter your road name.'),
                              _value=db_address.street)),
                    DIV(LABEL('City', _for='city'),
                        INPUT(_name='city',
                              requires=IS_NOT_EMPTY(error_message='Please enter your home city.'),
                              _value=db_address.city)),
                    DIV(LABEL('Country', _for='country'),
                        INPUT(_name='country',
                              requires=IS_NOT_EMPTY('Please enter your home country.'),
                              _value=db_address.country)),
                    DIV(LABEL('Post Code', _for='postcode'),
                        INPUT(_name='postcode',
                              requires=IS_MATCH('^\w{4} \w{3}$',
                                                error_message='Please enter your home postcode (i.e. VP22 6QZ).'),
                              _value=db_address.postcode, _placeholder='XXXX XXX'))),
                _class='long_div'),
                DIV(H3('Credit Details'),
                    DIV(LABEL('Card Number', _for='card_num'),
                        INPUT(_name='card_num', _type='number', _placeholder='XXXXXXXXXXXX',
                              requires=IS_MATCH('^\d{12}$', error_message='Please enter your 12 digit card number.'),
                              _value=db_credit.number)),
                    DIV(LABEL('Card Expiry Date', _for='expiry'),
                        INPUT(_name='expiry', _type='month',
                              requires=IS_DATE_IN_RANGE(format='%Y-%m', minimum=date.today(),
                                                        error_message='Please enter your current card\'s expiry date'),
                              _value=db_credit.expiry)),
                    DIV(LABEL('Security PIN', _for='pin'),
                        INPUT(_name='pin', _type='number', _min='000', _max='999',
                              requires=IS_MATCH('^\d{3}$', error_message='Please enter your card\'s CSC pin'),
                              _value=db_credit.PIN)),
                    DIV(H4('Billing Address'),
                        DIV(LABEL('House Name / Number', _for='bill_house'),
                            INPUT(_name='bill_house',
                                  requires=IS_NOT_EMPTY(error_message='Please enter your billing address house name / number.'),
                                  _value=db_credit_address.number)),
                        DIV(LABEL('Road name', _for='bill_street'),
                            INPUT(_name='bill_street',
                                  requires=IS_NOT_EMPTY(error_message='Please enter your billing address road name.'),
                                  _value=db_credit_address.street)),
                        DIV(LABEL('City', _for='bill_city'),
                            INPUT(_name='bill_city',
                                  requires=IS_NOT_EMPTY(error_message='Please enter the city of your billing address.'),
                                  _value=db_credit_address.city)),
                        DIV(LABEL('Country', _for='bill_country'),
                            INPUT(_name='bill_country',
                                  requires=IS_NOT_EMPTY(error_message='Please enter the country of your billing address.'),
                                  _value=db_credit_address.country)),
                        DIV(LABEL('Post Code', _for='bill_postcode'),
                            INPUT(_name='bill_postcode',
                                  requires=IS_MATCH('^\w{4} \w{3}$',
                                                    error_message='Please enter your billing address postcode (i.e. FF26 8PN).'),
                                  _value=db_credit_address.postcode, _placeholder='XXXX XXX'))),
                    _class='right_div'),
                DIV(INPUT(_type='submit', _value='Submit', _class='button_forward clear')),
                _id='split_form')
